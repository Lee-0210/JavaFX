	...
	public List<Board> list() {
		...
		try {
			...
			while( rs.next() ) {			// next() : 조회 결과의 다음 데이터로 이동
				...
				board.setCreatedAt( rs.getString(5) );
				board.setUpdatedAt( rs.getString(6) );
	...
	public Board select(int no) {
		...
		try {
			...
			if( rs.next() ) {
				...
				board.setWriter( rs.getString(3) );
				board.setContent( rs.getString(4) );
				board.setCreatedAt( sdf.format(rs.getTimestamp(5)));
				board.setUpdatedAt( sdf.format(rs.getTimestamp(6)));
			}
